...
program.version(SAGE2_version)
	...
	.option('--monitoring',			  'Performance monitoring', false);
// Parse the arguments
program.parse(args);
// Get the results
...
function initializeSage2Server() {
	...
	if (commander.trackUsers) {
		if (typeof commander.trackUsers === "string" && sageutils.fileExists(commander.trackUsers)) {
			users = json5.parse(fs.readFileSync(commander.trackUsers));
		} else {
			users = {};
		}
		users.session = {};
	...
	if (typeof commander.password  === "string" && commander.password.length > 0) {
		// Creating a new hash from the password
		global.__SESSION_ID = md5.getHash(commander.password);
		...
		sageutils.generateConnectionNote(uploadsDirectory, hostOrigin, commander.password);
	} else if (sageutils.fileExists(passwordFile) && (typeof commander.password  === "string")) {
...
function setupListeners(wsio) {
	...
	// Pdf thumbnail updates in UI as page turns
	wsio.on('pdfPageUpdateUiThumbnail',             wsPdfPageUpdateUiThumbnail);
}
...
function wsAppWindowCreated(wsio, data) {
	if (SAGE2Items.applications.list.hasOwnProperty(data.id) === true) {
		...
		// Only share if master display. Prevents multiple events when multiple display clients are used for a site.
		if (wsio === masterDisplay) {
...
function wsLoginUser(wsio, data) {
	...
	if (res.error === null) {
		...
		// HACK LUC
		wsRegisterInteractionClient(wsio, {name: data.SAGE2_ptrName, color: data.SAGE2_ptrColor});
...
function wsUpdateAppState(wsio, data) {
	...
	if (wsio === masterDisplay && SAGE2Items.applications.list.hasOwnProperty(data.id)) {
		...
		if (data.updateLocalDisplayClienst) {
...
function saveDrawingSession(data) {
	var filename = "drawingSession" + Date.now();
...
function getPathOfConfigFile() {
	...
	if (commander.configuration) {
		configFile = commander.configuration;
	} else {
...
// function updateRemoteSitesListFromConfigurationFile() {
	if (config.remote_sites) {
		...
		// Also need to remove the warningSite
		try {
			interactMgr.removeGeometry(VersionManager.remoteSiteBlockName, "staticUI");
		} catch (e) {
			// Ignoring case when it can't remove geometry. Means wasn't made.
		}
		...
		// Add the gemeotry for the button
		interactMgr.addGeometry(VersionManager.remoteSiteBlockName, "staticUI",
...
if (commander.session) {
	setTimeout(function() {
		...
		if (commander.session === true) {
			loadSession();
		} else {
			// if argument specified
			loadSession(commander.session);
		}
	}, 1000);
}
...
function wsCallFunctionOnApp(wsio, data) {
	...
	if (SAGE2Items.applications.list.hasOwnProperty(data.app)) {
		...
		if (data.func === "SAGE2DeleteElement") {
			...
			return;
		} else if (data.func === "SAGE2CloseAppDeleteFile") {
			deleteApplication(data.app, null, wsio);
			wsDeleteElementFromStoredFiles(wsio, data.parameters);
			return;
...
function wsSnippetSaveIntoServer(wsio, data) {
	...
	var filename = `${data.type}-${data.desc.replace(" ", "_")}-${Date.now()}.snip`;
...
/**
 * Relay the thumbnail of current pdf to UI clients.
 *
 * @method wsPdfPageUpdateUiThumbnail
 * @param {Object} wsio - ws to originator.
 * @param {Object} data - should have a data url for thumbnail that needs to be passed to UI clients
 */
function wsPdfPageUpdateUiThumbnail(wsio, data) {
	broadcast("pdfPageUpdateUiThumbnail", data);
}
