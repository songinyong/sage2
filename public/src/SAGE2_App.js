...
/* global storedFileList, addStoredFileListEventHandler, removeStoredFileListEventHandler, isBrowser */
...
var SAGE2_App = Class.extend({
	...
	*/
	...
	SAGE2Sync: function(updateRemote, updateLocalDisplayClienst = false) {
		this.SAGE2UpdateAppOptionsFromState();
		...
		if (isMaster) {
			...
			wsio.emit('updateAppState', {
				...
				updateLocalDisplayClienst: updateLocalDisplayClienst
			});
		} else if (isBrowser) {
	...
	*/
	getFullContextMenuAndUpdate: function() {
		...
		if (isMaster) {
			...
			// See if possible to close and delete file
			let fileObj = this.findFileWithinStoredFileList();
			if (fileObj) {
				appContextMenu.entries.push({
					description: "Close app and delete file",
					callback: "SAGE2CloseAppDeleteFile",
					parameters: {
						filename: fileObj.filename,
						application: fileObj.sage2Type
					}
				});
			}
	...
	*/
	...
	/**
	* Will search through stored files and attempt to identify filename which can be used with deleteElementFromStoredFiles
	*
	* Only works with the following apps
	*    pdf_viewer, movie_player, image_viewer, quickNote
	*
	* @method findFileWithinStoredFileList
	*/
	findFileWithinStoredFileList: function() {
		let fileNameNoPath; // Need path? problem is path won't match.
		let sflSubSection;
		let fnToCheck;

		// Not available if display is connecting after content already posted.
		if (!storedFileList) {
			// Not sure if there is a better way to do this. Basically wait for a second before trying again.
			setTimeout(() => {
		...
		switch (this.application) {
			case "pdf_viewer":
				fileNameNoPath = this.state.doc_url;
				fileNameNoPath = fileNameNoPath.substring(fileNameNoPath.lastIndexOf("/") + 1);
				sflSubSection = storedFileList.pdfs;
				break;
			case "movie_player":
				fileNameNoPath = this.state.video_url;
				fileNameNoPath = fileNameNoPath.substring(fileNameNoPath.lastIndexOf("/") + 1);
				sflSubSection = storedFileList.videos;
				break;
			case "image_viewer":
				fileNameNoPath = this.state.img_url;
				fileNameNoPath = fileNameNoPath.substring(fileNameNoPath.lastIndexOf("/") + 1);
				sflSubSection = storedFileList.videos;
				sflSubSection = storedFileList.images;
				break;
			case "quickNote":
				fileNameNoPath = this.state.file;
				fileNameNoPath = fileNameNoPath.substring(fileNameNoPath.lastIndexOf("/") + 1);
				// sflSubSection = storedFileList.others;
				sflSubSection = []; // force skip into others
				break;
			default:
				return false; // need to know filename....
		}
		// Just incase, decode uri
		fileNameNoPath = decodeURI(fileNameNoPath);

		// Try the main section first
		for (let i = 0; i < sflSubSection.length; i++) {
			// Get the file object
			fnToCheck = sflSubSection[i];
			fnToCheck = fnToCheck.filename;
			fnToCheck = fnToCheck.substring(fnToCheck.lastIndexOf("/") + 1);
			if (fnToCheck == fileNameNoPath) {
				return sflSubSection[i];
			}
		}

		// Just in case, check the others section for mislabled files.
		sflSubSection = storedFileList.others;
		for (let i = 0; i < sflSubSection.length; i++) {
			// Get the file object
			fnToCheck = sflSubSection[i];
			fnToCheck = fnToCheck.filename;
			if (fnToCheck == null) {
				continue; // links have blank filenames... is this a bug?
			}
			fnToCheck = fnToCheck.substring(fnToCheck.lastIndexOf("/") + 1);
			if (fnToCheck == fileNameNoPath) {
				return sflSubSection[i];
