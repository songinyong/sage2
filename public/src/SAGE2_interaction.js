...
/* global displayUI, pointerX, pointerY, SAGE2_webrtc_ui_tracker, SAGE2_speech */
...
function SAGE2_interaction(wsio) {
	...
	*/
	...
	/**
	* Handler for voice recognition mode
	*
	* @method changeVoiceReconitionModeMethod
	* @param value {Number} a value corresponding to mode
	* @param resolution {String} selection text
	*/
	this.changeVoiceReconitionModeMethod = function(value, resolution) {
		// Value might be a string.
		value = parseInt(value);
		let mode = (value == 3) ? "speech_ui_pointer_enabled" :
			((value == 2) ? "speech_ui_enabled" : "speech_disabled");

		SAGE2_speech.setVoiceRecognitionMode(mode);
	};
	...
	this.settingsDialog = function(type) {
		...
		switch (type) {
			...
			case 'main':
				...
				if (SAGE2_speech.serverEnabled) {
					elements.unshift(
						{view: "label", label: "Voice Recognition Settings", align: "center" },
						{
							type: "space",
							rows: [
								{
									view: "richselect",
									label: "Mode",
									id: "use_voiceRecognition",
									value: (this.voiceRecognitionMode) || 1,
									options: [
										{id: 1, value: "Disable Voice Recognition"},
										{id: 2, value: "Enable Voice Recognition in UI"},
										{id: 3, value: "Enable Voice Recognition in UI and Pointer Mode"}
									]
								}
							]
						}
		...
		if (type === 'main') {
			...
			let voice = $$("use_voiceRecognition");
			if (voice) {
				voice.attachEvent("onChange", () => {
					this.changeVoiceReconitionMode(voice.getValue(), voice.getText());
				});
			}
		}
	...
	*/
	...
	this.changeVoiceReconitionMode   = this.changeVoiceReconitionModeMethod.bind(this);
