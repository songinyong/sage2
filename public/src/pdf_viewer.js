...
var pdf_viewer = SAGE2_App.extend({
	...
	*/
	init: function(data) {
		...
		// array containing the image links
		this.thumbnailsLink = [];
		// array containing the image data urls
		this.thumbnailsDataUrl = [];
		...
		pdfFile.getPage(pageNumber).then(function(page) {
			...
			page.render(renderContext).promise.then(function(pdf) {
				...
				if (thumbnail) {
					...
					that.thumbnailsDataUrl[page.pageNumber] = durl;
	...
	*/
	...
	goToPage: function(page) {
		...
		if (this.state.currentPage !== page) {
			...
			// Tell UI to show page thumbnail
			if (this.thumbnailsDataUrl[page]) {
				wsio.emit("pdfPageUpdateUiThumbnail", {id: this.id, thumbnail: this.thumbnailsDataUrl[page]});
			}
		}
