...
/* global showSAGE2Message webkitSpeechGrammarList displayUI interactor*/
...
SAGE2_speech.serverEnabled      = true;
SAGE2_speech.userAllowed        = true;
SAGE2_speech.userAllowedPointer = true;
	...
	if (!("webkitSpeechRecognition" in window)) {
		...
		this.webkitSR.onerror = function(e) {
			...
			if (e.error === "not-allowed") {
				...
				SAGE2_speech.userAllowed = false;
				SAGE2_speech.serverEnabled = false;
				SAGE2_speech.setVoiceRecognitionMode("speech_disabled");
			}
		};
...
SAGE2_speech.enableMouseholdToStart = function(e) {
	...
	if (dx < SAGE2_speech.mouseHoldMoveLimit.x && dy < SAGE2_speech.mouseHoldMoveLimit.y) {
		...
		if (!SAGE2_speech.firstNameMention) {
			...
			setTimeout(function() {
				if (SAGE2_speech.userAllowed && SAGE2_speech.pointerCompliant()) {
...
SAGE2_speech.setVoiceRecognitionMode = function(mode) {
	switch (mode) {
		// case 1: // disabled
		case "speech_disabled":
			addCookie("SAGE2_speech_allowed", false);
			SAGE2_speech.userAllowed = false;
			interactor.voiceRecognitionMode = 1;
			break;
		// case 2: // ui enabled
		case "speech_ui_enabled":
			addCookie("SAGE2_speech_allowed", true);
			SAGE2_speech.userAllowed = true;
			addCookie("SAGE2_speech_allowedPointer", false);
			SAGE2_speech.userAllowedPointer = false;
			if (!SAGE2_speech.hasInit) {
				SAGE2_speech.init();
			}
			interactor.voiceRecognitionMode = 2;
			break;
		// case 3: // enabled + pointer
		case "speech_ui_pointer_enabled":
			addCookie("SAGE2_speech_allowed", true);
			addCookie("SAGE2_speech_allowedPointer", true);
			SAGE2_speech.userAllowed = true;
			SAGE2_speech.userAllowedPointer = true;
			if (!SAGE2_speech.hasInit) {
				SAGE2_speech.init();
			}
			interactor.voiceRecognitionMode = 3;
			break;
	}
};
...
SAGE2_speech.setNameMarker = function (nameMarkerFromServer) {
	...
	let startingMode = "speech_disabled";
	// If voice is enabled, begin init
	if (displayUI.config.voice_commands.enabled) {
		SAGE2_speech.serverEnabled = true;
		SAGE2_speech.init();

		// Check if user allowed preferences exist.
		// Bool check defaults to true if cookie doesn't exist
		SAGE2_speech.userAllowed = getCookie("SAGE2_speech_allowed") === "false" ? false : true;
		// Bool cehck defaults to false if cookie doesn't exist
		SAGE2_speech.userAllowedPointer = getCookie("SAGE2_speech_allowedPointer") === "true" ? true : false;

		if (SAGE2_speech.userAllowed) {
			startingMode = "speech_ui_enabled";
			if (SAGE2_speech.userAllowedPointer) {
				startingMode = "speech_ui_pointer_enabled";
			}
		}
	} else {
		SAGE2_speech.serverEnabled = false;
	...
	SAGE2_speech.setVoiceRecognitionMode(startingMode);
};
...
/**
 * Checks if in pointer mode and if should allow starting in pointer mode
 *
 * @method pointerCompliant
 */
SAGE2_speech.pointerCompliant = function () {
	// Check if in pointer mode:
	let inPointerMode = document.getElementById("sage2pointerDialog").style.display;
	inPointerMode = (inPointerMode == "block") ? true : false;

	// (True if not in pointer mode) or (in pointer mode and allowed)
	if (!inPointerMode) {
		return true;
	} else if (inPointerMode && SAGE2_speech.userAllowedPointer) {
		return true;
	}
	return false;
};
