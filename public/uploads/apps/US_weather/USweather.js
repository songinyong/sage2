...
var USweather = SAGE2_App.extend({
	...
	newWeather: function(data) {
		...
		console.log(data.weatherOut.list[0]);

		if ((weatherOut === null) || (weatherOut.list === null) || (weatherOut.list[0].main === null) ||
			(weatherOut.list[0].main.temp === null) ||
			(weatherOut.list[0].weather[0].icon === null)) {
			return;
		}

		weather = parseInt(weatherOut.list[0].main.temp);

		weatherImage = "https://www.evl.uic.edu/aej/TEMPS/icons2/" +
			weatherOut.list[0].weather[0].icon + ".svg";

		console.log(weatherImage);

		if ((weatherImage === null) || (weatherImage === "null")) {
			return;
		}

		this.drawEverything(lat, lon, weather, weatherImage);
	},
	...
	updateOutsideTemp: function() {
		...
		var url_part0 = "https://api.openweathermap.org/data/2.5/find?lat=";
		var url_part1 = "";
		var url_part2 = "&APPID=e384a6c484feb61cdc310f2b085826d5&units=imperial&cnt=1";


		// now in 2019 with openweather rather than yahoo ...
		// https://api.openweathermap.org/data/2.5/find?lat=55.5&lon=37.5&APPID=e384a6c484feb61cdc310f2b085826d5&units=imperial&cnt=1
		...
		function getData(plat, plon, preplace) {
			...
			plon = plon.toFixed(3);
			plat = plat.toFixed(3);
			d3.json(url_part0 + url_part1 + plat + "&lon=" + plon + url_part2,
